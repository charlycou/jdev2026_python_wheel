...
-> Uncomment and complete the lines in the pyproject.toml related to package-data. This in important to include the data files in the wheel.

# 5th step: Build the package locally:

In a new virtual envienvironment, we can try to build the package locallywith the following commands:
...
No, we are able to build the wheel locally and, we can commit the code to the repository.  
...
```bash
pip install twine
twine upload --repository testpypi dist/* # paste your API token is needed
```
...
-> From the model, complete the file .github/workflows/publish.yml and modify it to fit your package and branch name. You will need to add your API token as a secret in the Github repository settings called TEST_PYPI_API_TOKEN. Commit, and push everything on Github and check if the workflow is triggered and if the package is published on test PyPI. Maybe tag a new version of your package to trigger the workflow.
Maybe you need to change the default branch of the repository to the branch where you are working (workshop_starting_point) to be able to trigger the workflow when you push a new tag (Github security).
